...
public class MazesController : ControllerBase
{
    ...
    public MazesController()
    {
        ...
        _fontCollection = new();
        using Stream stream = assembly.GetManifestResourceStream("HugeMazes.Web.SecularOne-Regular.ttf")!;
    ...
    public ActionResult GenerateMazeWithPath(int width, int height)
    {
        ...
        w.Restart();
        using var imageStream = new MemoryStream();
        using var image = HugeMazes.Render(new StreamStore(imageStream), maze, path);
        image.Write();
    ...
    public ActionResult GenerateMazeWithPathSeed(int seed, int width, int height)
    {
        ...
        using var image = HugeMazes.Render(new StreamStore(memoryStream), maze, path);
        image.Write();
        var toImageTime = w.Elapsed;
